...
#ifdef _DEBUG
   char VersionString[] = "VERSION 1.3 DEBUG"; // (pre-release; build 100)";
#else
   char VersionString[] = "VERSION 1.3 PRE-RELEASE"; // (pre-release; build 100)";
#endif
...
again_heimatflughafen:
      ...
      else if (PageNum==MULTIPLAYER_SELECT_NETWORK)  //Netzwerk: Provider-Medium wählen
      {
         ...
         else if (GridPos.IfIsWithin (17, 15, 24, 15))
         {
            ...
            if (id==SBNETWORK_RAKNET_DIRECT_JOIN || id == SBNETWORK_RAKNET_NAT_JOIN)
            {
               gHostIP=".";
